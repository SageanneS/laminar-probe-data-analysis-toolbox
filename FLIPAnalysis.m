%%
%  FLIPAnalysis.m is not an original file. The original creators of this
%  file are cited here: 
...
if ~(setfreqbool)
    ...
    if sum(nanboolean)==0
        ...
        low_freq = ceil([1,10, 20, 30, 40, 50] / freqbinsize);

        %gamma_freq = ceil([30, 40, 50, 60, 70, 80,90,100,110,120,130,140,150 ] / freqbinsize);
        gamma_freq = ceil([50, 60, 70, 80,90,100,110,120,130,140,150 ] / freqbinsize);
...
xlim([0 size(relpow,2)]); ylim([1 size(relpow,1)]); title('LFP Relative Power; \beta-Freq Range:blue \gamma-Freq Range:red');
...
% plot low freq range
line([lowfreqrange(1) lowfreqrange(1)],get(gca,'YLim'),'LineWidth',1.5,'Color','b','LineStyle','--');
line([lowfreqrange(2) lowfreqrange(2)],get(gca,'YLim'),'LineWidth',1.5,'Color','b','LineStyle','--');
% plot low freq range
line([highfreqrange(1) highfreqrange(1)],get(gca,'YLim'),'LineWidth',1.5,'Color','r','LineStyle','--');
line([highfreqrange(2) highfreqrange(2)],get(gca,'YLim'),'LineWidth',1.5,'Color','r','LineStyle','--');

line(get(gca, 'xlim'), [crossoverchannel crossoverchannel], 'LineWidth', 2.0, 'Color', 'w', 'LineStyle', '-.');
text(size(relpow,2), crossoverchannel, 'Crossover Channel', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'Color','w', 'FontWeight', 'bold', ...
    'FontSize', 13);
line(get(gca, 'xlim'), [lowfreqmaxchannel lowfreqmaxchannel], 'LineWidth', 2.0, 'Color', 'w', 'LineStyle', '-.');
text(size(relpow,2), lowfreqmaxchannel, 'Alpha/Beta Peak Channel', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'Color','w', 'FontWeight', 'bold', ...
    'FontSize', 13);
line(get(gca, 'xlim'), [highfreqmaxchannel highfreqmaxchannel], 'LineWidth', 2.0, 'Color', 'w', 'LineStyle', '-.');
text(size(relpow,2), highfreqmaxchannel, 'Gamma Peak Channel', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'Color','w', 'FontWeight', 'bold', ...
    'FontSize', 13);
set(gcf, 'color', 'w');
...
line(get(gca, 'xlim'), [crossoverchannel crossoverchannel], 'LineWidth', 1.5, 'Color', 'k', 'LineStyle', '-.');
text(1, crossoverchannel, 'Crossover Channel', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'FontSize', 13);
line(get(gca, 'xlim'), [lowfreqmaxchannel lowfreqmaxchannel], 'LineWidth', 1.5, 'Color', 'k', 'LineStyle', '-.');
text(1, lowfreqmaxchannel, 'Alpha/Beta Peak Channel', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'FontSize', 13);
line(get(gca, 'xlim'), [highfreqmaxchannel highfreqmaxchannel], 'LineWidth', 1.5, 'Color', 'k', 'LineStyle', '-.');
text(1, highfreqmaxchannel, 'Gamma Peak Channel', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'FontSize', 13);
...
set(gcf,'Position',[200 200 1150 700]);
set(gcf, 'color', 'w');
...
uiwait(helpdlg('View and Save the figure, then click OK to continue.'));
end
